...
2. `bun --filter 'server' setup-dev`
    ...
    - Alternatively, if you don't want the database seeded: `bun --filter 'server' db-create`
...
2. To start the database and API endpoint: `bun --filter 'server' dev`
    - A lightweight database dashboard is available at https://local.drizzle.studio/.

3. To start the web application: `bun --filter 'client' dev`
...
To get started, run `bun --filter 'client' storybook`

## Tests

When possible, write robust tests for new features and make sure tests for existing features still pass by running `bun test`.
...
When the work on an is completed, open a pull request and complete the relevant sections of the template. An approving review and all tests passing is required before a PR can be merged.
